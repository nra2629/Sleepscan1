"""
SleepScan — Flask App
Memory optimised — lazy loading of heavy libraries
"""

from flask import Flask, render_template, request, jsonify, send_file
import os, io, zipfile, gc
    ...
    try:
        ...
        if not rec_files or not ann_files:
            if len(edf_names) >= 2:
                ...
                return jsonify({'error': f'Could not find both EDF files. Found: {edf_names}'}), 400
        ...
        # Free signal memory immediately after feature extraction
        del signals
        gc.collect()
        ...
        result = {
            ...
            'cluster_desc':  {'label': cluster_desc.get('label',''), 'bullets': cluster_desc.get('bullets',[]), 'desc': cluster_desc.get('desc','')},
...
def recalculate():
    try:
        ...
        clinic       = request.form.get('clinic', '')
        night        = request.form.get('night', 'Night 1')
        stages       = json.loads(request.form.get('stages', '[]'))
        epoch_times  = json.loads(request.form.get('epoch_times', '[]'))
